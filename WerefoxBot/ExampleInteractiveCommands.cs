    ...
    public class ExampleInteractiveCommands : BaseCommandModule
    {
        ...
        public async Task WaitForReaction(CommandContext ctx)
        {
            ...
            if (!em.TimedOut)
            {
                ...
                var embed = new DiscordEmbedBuilder
                {
                    Color = em.Result.Message.Author is DiscordMember m ? m.Color : new DiscordColor(0xFF00FF),
                    Description = em.Result.Message.Content,
                    Author = new DiscordEmbedBuilder.EmbedAuthor
                    {
                        Name = em.Result.Message.Author is DiscordMember mx ? mx.DisplayName : em.Result.Message.Author.Username,
                        IconUrl = em.Result.Message.Author.AvatarUrl
                    }
                };
        ...
        public async Task SendPaginated(CommandContext ctx)
        {
            ...
            // send the paginator
            await interactivity.SendPaginatedMessageAsync(ctx.Channel, ctx.User, lipsum_pages, null, PaginationBehaviour.Ignore , PaginationDeletion.KeepEmojis , TimeSpan.FromMinutes(5));
